...
u8 SD_Init(void)
{
 ...
 // SPI_SetSpeed(SPI_SPEED_LOW);
  ...
  do
  {
    ...
    retry++;
   // printf("r1 is %d\r\n", r1);
  }while((r1 != 0x01) && (retry<500));
  //跳出循环后，检查原因：初始化成功？or 重试超时？
  if(retry==500) return 1;   //超时返回1

  //-----------------SD卡复位到idle结束-----------------
  //获取卡片的SD版本信息
  r1 = SD_SendCommand_NoDeassert(CMD8, 0x1aa,0x87);
 // printf("ri is at idle %d\r\n", r1);
